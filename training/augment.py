...
## AugmentPipe is constructed and applied when calculating accumulate_gradients() in loss.py when running D (see run_D)

## the following kwargs are read from augment_kwargs upon reconstructiong AugmentPipe class:
# "xflip": 1,
# "rotate90": 1,
# "xint": 1,
# "scale": 1,
...
class AugmentPipe(torch.nn.Module):
    ...
    def forward(self, images, debug_percentile=None):
        ...
        # h: self.p is set to the value of augment_p when constructing the class in the training loop which is the aug probability 
        # -------------------------------------
        ...
        if self.xflip > 0:
            ...
            # torch.rand() --> uniform random from [0,1) then *2 --> [0,2) then floor makes them 0 or 1.
            # so the output is a tensor of size batch_size with 0 or 1 values.
        ...
        # Apply isotropic scaling with probability (scale * strength). # h: strength is self.p which is augment_p
        if self.scale > 0:
